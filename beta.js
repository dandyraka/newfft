...
function add (command) {
  return Util.ask('username? ')
	...
	.then(()=>Util.ask('auto DM (Y/N)? '))
    .then(()=>Util.ask('delay (ms)? '))
    .then((rl) => {
      login(Util.responses[0], Util.responses[1])
        .then(function(session) {
		  if(Util.responses[3] == "y" || Util.responses[3] == "Y"){
			var saveDM = "Yes";
		  } else {
			var saveDM = "No";
		  }
                    ...
                    .then(function (resp) {
                      ...
                      Client.Like.create(gSession, media[0].id)
                        .then(function (resp) {
						  ...
						  if(userInput.DM == "Yes"){
							  Client.Thread.configureText(gSession, following[i].id, sample(comments).text)
								.then(function (resp) {
									console.log(`${current._params.username} ${Colors.FgGreen}DM Added${Colors.Reset}`);
								}).catch(function (error) {
								  console.log(error);
								});
						  }
